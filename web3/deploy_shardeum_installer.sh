...
# 这个脚本改自：https://raw.githubusercontent.com/shardeum/validator-dashboard/main/installer.sh 
# 节点部署成功之后访问webui会报错，不知道为什么官方github没回复。
...
command -v git >/dev/null 2>&1 || {
    echo >&2 "'git' is required but not installed."
    exit 1
}
command -v docker >/dev/null 2>&1 || {
    echo >&2 "'docker' is required but not installed. See https://github.com/shardeum/validator-dashboard?tab=readme-ov-file#how-to-install-and-run-a-shardeum-validator-node for details."
    exit 1
}
...
git clone https://github.com/shardeum/validator-dashboard.git ${NODEHOME} || {
    echo "Error: Permission denied. Exiting script."
    exit 1
}
cd ${NODEHOME}
chmod a+x ./*.sh
