...
# debian 系统
# 部署taiko验证者节点，命令行: curl -sSL https://raw.githubusercontent.com/superjagger/deploy/main/web3/debian_deploy_taiko.sh | bash -s -- "钱包地址" "私钥" "Ethereum Holskey HTTP RPC连接" "Ethereum Holskey WS RPC连接" "Beacon Holskey RPC连接" "prover_endpoints 地址" "不输入或者0表示保留数据，否则是清空数据"
# 可以用下面的公共rpc地址
...
sed -i "s|PORT_L2_EXECUTION_ENGINE_METRICS}:6060\"|PORT_L2_EXECUTION_ENGINE_METRICS}:6061\"|" docker-compose.yml
